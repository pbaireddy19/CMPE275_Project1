...
public class Server {
	...
	private boolean verifyConf(ServerConf conf) {
		...
		if (conf == null) {
			...
			return false;
		} else if (conf.getServer().getNodeId() < 0) {
			logger.error("Bad node ID, negative values not allowed.");
			rtn = false;
		} else if (conf.getServer().getPort() < 1024 || conf.getServer().getMgmtPort() < 1024) {
	...
	private static class StartCommunication implements Runnable {
		...
		public void run() {
			...
			try {
				...
				bootstrap.put(conf.getServer().getPort(), b);
				...
				// Start the server.
				logger.info("Starting server " + conf.getServer().getNodeId() + ", listening on port = " + conf.getServer().getPort());
				ChannelFuture f = b.bind(conf.getServer().getPort()).syncUninterruptibly();
	...
	private static class StartManagement implements Runnable {
		...
		public void run() {
			...
			try {
				ServerBootstrap b = new ServerBootstrap();
				bootstrap.put(conf.getServer().getMgmtPort(), b);
				...
				logger.info("Starting mgmt " + conf.getServer().getNodeId() + ", listening on port = " + conf.getServer().getMgmtPort());
				ChannelFuture f = b.bind(conf.getServer().getMgmtPort()).syncUninterruptibly();
	...
	private void startManagers() {
		...
		for (NodeDesc nn : conf.getAdjacent().getAdjacentNodes().values()) {
			...
			// fn(from, to)
			HeartbeatPusher.getInstance().connectToThisNode(conf.getServer().getNodeId(), node);
			
			ForwardResource.getInstance().setCfg(conf);
		...
		logger.info("Server " + conf.getServer().getNodeId() + ", managers initialized");
	}
	...
	public void run() {
		...
		logger.info("Initializing server " + conf.getServer().getNodeId());
		...
		logger.info("Server " + conf.getServer().getNodeId() + " ready");

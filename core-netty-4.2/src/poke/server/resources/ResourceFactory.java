...
public class ResourceFactory {
	...
	// Total number of active nodes in the system
	private int nodesInSystem = 0;
	...
	public Resource resourceInstance(Header header) {
		...
		if (header.hasToNode()) {
			if (cfg.getServer().getNodeId() == header.getToNode())
	...
	// methods to access and modify Active Node Count
	
	public int getNodesInSystem() {
		return nodesInSystem;
	}
	
	public void incrementNodesInSystem() {
		nodesInSystem++;
	}
	
	public void decrementNodesInSystem() {
		nodesInSystem--;
	}
}

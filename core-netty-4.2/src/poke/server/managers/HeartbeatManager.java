...
public class HeartbeatManager extends Thread {
	...
	private Management generateHB() {
		...
		mhb.setOriginator(HeartbeatManager.conf.getServer().getNodeId());
				...
				if (outgoingHB.size() > 0) {
					...
					for (HeartbeatData hd : outgoingHB.values()) {
						...
						try {
							...
							//if (logger.isDebugEnabled())
							logger.info("beat (" + HeartbeatManager.conf.getServer().getNodeId() + ") sent to "
	...
	public class CloseHeartListener implements ChannelFutureListener {
		...
		public void operationComplete(ChannelFuture future) throws Exception {
			if (outgoingHB.containsValue(heart)) {
				...
				// Modified as the configuration is a bidirectional mesh
				// Decrementing number of nodes in system
				...
				if (incomingHB.containsValue(heart)) {
					...
					ResourceFactory.getInstance().decrementNodesInSystem();
				}
			}
		}
	}

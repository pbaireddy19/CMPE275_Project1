...
public class ElectionManager implements ElectionListener {
	...
	public void startElection() {
		...
		elb.setHops((conf.getServer().getMgmtPort() % 100));
		elb.setDesc("Initial Election");
		elb.setCandidateId(conf.getServer().getNodeId()); // promote self
		...
		logger.info("Election started by node " + conf.getServer().getNodeId());
	...
	public void processRequest(Management mgmt) {
		...
		if (req.getAction().getNumber() == LeaderElection.ElectAction.WHOISTHELEADER_VALUE) {
			...
			logger.info("Node " + conf.getServer().getNodeId() + " got an answer on who the leader is. Its Node "
					+ req.getCandidateId());
		...
		if (req.getAction().getNumber() == LeaderElection.ElectAction.DECLAREELECTION_VALUE)
		{
			...
			createElection().process(mgmt);
		}
		else if (election != null)
		     election.process(mgmt);
		else
	...
	public void assessCurrentState(Management mgmt) {
		...
		if (firstTime > 0 && ConnectionManager.getNumMgmtConnections() > 0) {
			...
			this.firstTime--;
			askWhoIsTheLeader();
		} else if (leaderNode == null && (election == null || (i == -1))) {
	...
	public void concludeWith(boolean success, Integer leaderID) {
		if (success) {
			...
			if(this.leaderNode == conf.getServer().getNodeId()){

				// create cluster object and than send it to cluster service implementation

				ClusterMapperManager.setClusterDetails( DbConfigurations.getIp(), conf.getServer().getPort());
	...
	private void respondToWhoIsTheLeader(Management mgmt) {
		...
		logger.info("Node " + conf.getServer().getNodeId() + " is replying to " + mgmt.getHeader().getOriginator()
	...
	private void askWhoIsTheLeader() {
		logger.info("Node " + conf.getServer().getNodeId() + " is searching for the leader");
	...
	private Election createElection() {
		if (election == null) {
			synchronized (syncPt) {
				// new election
				String clazz = "poke.server.election.FloodMaxElection";//ElectionManager.conf.getElectionImplementation();
				...
				try {
					...
					election.setNodeId(conf.getServer().getNodeId());
					election.setListener(this);
					election.setConf(this.conf);


					// this sucks - bad coding here! should use configuration
					// properties
					if (election instanceof FloodMaxElection) {
						//logger.warn("Node " + conf.getServer().getNodeId() + " setting max hops to arbitrary value (4)");
	...
	public void leaderFailureCheck()
	{
		logger.info(" In the leader failure the leader node is ------>> " + leaderNode);
		if(leaderNode != null)
		{
			if(!HeartbeatManager.getInstance().getIncomingHB().contains(leaderNode))
			{

				logger.info("Leader: " + leaderNode + " is no longer Active");
				leaderNode = null;
				election = null;
				if(ConnectionManager.getNumMgmtConnections() != 0)
				{
					i = -1;
					logger.info("Starting Election for New Leader");
					startElection();
				}	
			}
		}
		...
		{
			logger.info("Leader Not Declared Yet");
		}
	}
}

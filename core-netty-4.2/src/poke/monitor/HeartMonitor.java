...
public class HeartMonitor {
	...
	// returning channel
		...
		// returning the node id to the Election Manager
	...
	public boolean startHeartbeat() {
		...
		try {
			...
			rtn = true;
				// calling the addAdjacentNodeChannel
			// also incrementing the nodes in the system
			if (rtn){
				HeartbeatManager.getInstance().addAdjacentNodeChannel(iamNode, ch, ch.remoteAddress());
				ResourceFactory.getInstance().incrementNodesInSystem();
			}
	...
	public static class MonitorClosedListener implements ChannelFutureListener {
		...
		public void operationComplete(ChannelFuture future) throws Exception {
			monitor.release();
			// When a channel gets closed, check whether it is the leader who has failed
			ElectionManager.getInstance().leaderFailureCheck();
		}
	}
}
